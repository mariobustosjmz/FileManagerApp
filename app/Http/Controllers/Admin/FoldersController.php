...
use App\Folder;								//cargar con el namespace el Modelo que se pretende utilizar ,obtener data o guardar.
use App\Http\Requests\FoldersRequest;		//carga del request personalizado con las reglas especificas.
...
class FoldersController extends Controller {
	...
	public function __construct()
	{
	    ...
	    $this->module="Folders";	//definir de forma globar la varible que contiene el nombre del modulo.
	}
	...
	public function index() {
		...
		$folders = Folder::orderBy('id', 'DESC')->paginate(10);	//asignar a objeto 'extensions' el result de la consulta paginada de Extension.

		return view('admin.folders.index')->with('folders', $folders);
	 ...
	 */
	public function store(FoldersRequest $request) {

		//dd($request->all());
		$folder = new Folder($request->all());	//Objeto con nueva instancia del Modelo a utilizar (Folder) pasandole los datos del request con ->all();
		$folder->save();	//disparar el methodo save que guarda en BD los datos del objeto creado.
		Flash::success('The Folder <strong>' . $folder->name . '</strong> was created successfully');	//cargar un mensaje personalizado en la vista al redireccionar

		self::RegisterLog('Saved ' , $folder->name ,$this->module ,Auth::user()->id);	//disparar methodo con parametros para guardar log.

		return redirect()->route('folders.index');	//redireccionar a la ruta especificada.
	}
	...
	public function destroy($id) {
		...
		$folder = Folder::find($id);	//Objeto con el resultado de la busqueda(find) de un registro con un id en especifico.
		$folder->delete();	//borrar registro sobre el objeto previamente encontrado.
		Flash::warning('Deleted Folder <strong>' . $folder->name . '</strong>  ');	//cargar un mensaje personalizado en la vista al redireccionar
		self::RegisterLog('Deleted ' , $folder->name ,$this->module ,Auth::user()->id);	//disparar methodo con parametros para guardar log.
	...
	public function edit($id) {
		...
		$folder = Folder::find($id);	//Objeto con el resultado de la busqueda(find) de un registro con un id en especifico.
		return view('admin.folders.edit')->with('folder', $folder);	//pasar a la vista los dato de la extension para mostrar en el formulario al editar
	...
	public function update(Request $request, $id) {
		...
		$folder = Folder::find($id);	//Objeto con el resultado de la busqueda(find) de un registro con un id en especifico.
		$folder->fill($request->all());	//'llenar' al objeto especifico los datos del formulario con los datos por actualizar
		$folder->save();	//guardar ese nuevo objeto en la BD (remplazo/actualizacion)
		Flash::info('The Folder <strong>' . $folder->name . '</strong> was updated ');	//cargar un mensaje personalizado en la vista al redireccionar.
		self::RegisterLog('updated ' , $folder->name,$this->module  ,Auth::user()->id);	//disparar methodo con parametros para guardar log

		return redirect()->route('folders.index');	//redireccionar
